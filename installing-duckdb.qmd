...
Under the main VS menu, go to *Settings* -\> *Keyboard Shortcuts*, search for `run selected`, and choose the optoin `Terminal:run selected text in active terminal`. 

![Screenshot of the keyboard shortcut settings](img/vsc_shortcut.png)

- Set the shift-enter keypress by double-clicking. 
- Right-click on the *When* column, select *Change When Expression*, and add `editorLangId == 'sql'` to restrict to just SQL files and not every kind of file.

![Screenshot of the keyboard shortcut settings with the restriction applying it to SQL script only](img/vsc_shortcut_sql.png)
