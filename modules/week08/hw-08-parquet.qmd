...
- Import State and County names from the `fips_state_county.csv`, saving it as a table named `Fips` (see below on how to access the files).
- Load the Walkability Index data for your selected state from `walkability_wgs84.parquet` into a table named `Walkability_mystate` (for example, `Walkability_ca` for California; see below on how to access the files).
...
**Note that all the necessary files for this assignment are available on Workbench**. See below for how to access them via the web.
