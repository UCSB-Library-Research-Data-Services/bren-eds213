...
- Step 5. Deal with a numeric bugaboo.  If you look at the minimum and maximum values of longitude in the Site table, you will see that they range from -164.9 to 170.6.  But in this case, the correct interpretation of a positive longitude is really that it is to the west of the +/-180 meridian, i.e., it is even more negative than -180.  In other words, if a longitude $l$ is positive, it should be replaced by $l-360$, but if it is negative, it can be left as is.  Use a [CASE expression](https://duckdb.org/docs/current/sql/expressions/case) to express this.  Modify your query yet again so that it resembles:
