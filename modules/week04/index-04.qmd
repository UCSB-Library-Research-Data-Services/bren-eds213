...
```sql
CREATE TEMP TABLE Camp_assignment_tmp AS
...
SELECT Year, Site, Name 
   FROM Camp_assignment_tmp JOIN Personnel ON Observer = Abbreviation;
```
...
```sql
CREATE TEMP TABLE Camp_personnel AS
   SELECT Year, Site, Name 
   FROM Camp_assignment_tmp JOIN Personnel ON Observer = Abbreviation;
```
...
```sql 
SELECT * FROM Camp_assignment_tmp WHERE Site == 'bylo';
```

OK, we are ready to delete:

```sql 
DELETE FROM Camp_assignment_tmp WHERE Site == 'bylo';
```
