...
Although those functions look similar, they behave very differently; `COUNT(*)` count the number of rows in a table, group, ... without caring about the values. `COUNT(col)` does look at the values and will thus ignore NULL values.
   ```sql
...
```sql
SELECT * FROM A AS A1 JOIN A AS A2 ON A1.acol1 <> A2.bcol1;
```
...
```
`
Note: Would a `CROSS JOIN` make any sense here? Well we can still do it :)
