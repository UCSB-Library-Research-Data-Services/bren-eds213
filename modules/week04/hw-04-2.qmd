...
The Camp_assignment table lists where each person worked and when. Your goal is to answer, Who work with whom? That is, find all pairs of people who worked at the same site, and whose date ranges overlap while at that site. This can be solved using a self-join.
...
-   Step 2. We've matched up people working at the same site, but they didn't necessarily overlap in time. To the previous `ON` clause, add another condition that checks that the "A" person's and the "B" person's date ranges overlap. If you are unsure of the formula for this, it may be helpful to consult [this StackOverflow post](https://stackoverflow.com/questions/325933/determine-whether-two-date-ranges-overlap/325964). Your new table should be down to 3500 rows.

-   Step 3. Well, there's a problem. To better see the problem, add a clause `WHERE A.Site = 'lkri'` so you're only looking at 8 rows. If you look closely, you can see that there are rows that represent people working with themselves, which is not what we're interested in.  Also, every pair of people working together is represented by two rows, with the names in different order. We only want one row per unique pair.  We can clean this up by adding a condition to the clause, `A.Observer < B.Observer`.  I.e., we only want ordered, distinct pairs.  Your table should be down to $2$ rows.
